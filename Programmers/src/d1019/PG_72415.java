...
풀이법 : 
	1. 카드는 최대 6개이므로 순열을 생성하여 완전탐색하였습니다.
	2. 카드를 뒤집는 순서에 따라서도 다른 경우이기 때문에 순열은 2*n 배열로 선언해줍니다.
	3. 카드를 뒤집을 순열을 생성한 뒤 BFS를 사용해 최소 비용을 갱신합니다.
	...
	int findCard() {
		...
		for (int[] next : order) {
			...
			while (!q.isEmpty()) {
				...
				// 목표 좌표까지 도달했으면 비용 +1(뒤집는 비용) 후 cost 증가
